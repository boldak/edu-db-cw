...
@startuml
  ...
  usecase Dmanage2 as "<b>DP_2</b>\nКерувати файлами у датасетах" #32CD32
...
@startuml
    ...
    |Система|
        ...
        :сортує знайдені датасети за певним критерієм;
        :видає сторінку перегляду датасету;
    |Споживач даних|
...
@startuml
    ...
    |Система|
        ...
        : виконує пошук датасету за текстом;
        note right #ffaaaa
        <b> DС_EX2_1.4 Жодного датасету за даними
        <b>критеріями пошуку не знайдено.
        end note
        :фільтрує знайдені дані за обраними споживачем даних критеріями;
        :сортує знайдені датасети за певним критерієм;
        :видає сторінку перегляду датасетiв;
    |Споживач даних|
        :успішно переглядає дані;
        stop;
